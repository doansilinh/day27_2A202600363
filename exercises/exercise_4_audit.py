...
The implementation emits one AuditEntry per meaningful event:
analyze/route/commit/auto_approve/synthesize, plus before/after rows for
human_approval and escalate.
"""
...
async def node_analyze(state):
    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        a: PRAnalysis = calibrate_analysis(await llm.ainvoke([
            {"role": "system", "content": (
...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        reason=f"Routed to {decision} using confidence threshold policy",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    return {"decision": decision, "route_confidence": c}
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason="Paused for reviewer approval",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    await audit(state, AuditEntry(
        ...
        reason=resp.get("feedback") or "Reviewer resumed without additional feedback",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
def _render_comment_body(state) -> str:
    a = state["analysis"]
    display_confidence = state.get("route_confidence", a.confidence) if state.get("escalation_answers") else a.confidence
    lines = [f"### Automated review (confidence {display_confidence:.0%})", "", a.summary, ""]
    for c in a.comments:
...
async def node_commit(state):
    ...
    a = state["analysis"]
    if action == "committed":
        decision = "approve" if state.get("human_choice") == "approve" else "escalate"
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"High confidence auto path completed with action: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=answer_summary or "Reviewer submitted escalation answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
